...
module entraining_qe_moist_convection_mod
  
  !----------------------------------------------------------------------
  ! This module implements the simple quasi-equilibrium convection
  ! scheme
  ...
  ! 
  ! modified by Margaret Duffy (MLD) to include entrainment when entpar > 0
  !
  !----------------------------------------------------------------------
  ...
  public  entraining_qe_moist_convection, entraining_qe_moist_convection_init, entraining_qe_moist_convection_end

  !-----------------------------------------------------------------------
  !   ---- version number ----

  character(len=128) :: version = '$Id: qe_moist_convection.f90,v 1 2010/09/30 00:00:00 fms Exp $'
  ...
  real    :: entpar = 0. ! added by MLD
  real    :: entmax = 1.e-2 ! added by MLD
  ...
  namelist /entraining_qe_moist_convection_nml/  tau_bm, rhbm, Tmin, Tmax, val_inc, entpar, entmax ! entpar, entmax added by MLD
  ...
  !
  !  rhbm      = reference relative humidity toward which convection
  !  relaxes
  !
  ...
  ! 
  !  entpar    = unitless entrainment parameter, entrainment rate varies with entpar/z,
  !              entpar = 0 limit means no entrainment and convection scheme is same as
  !              qe_moist_convection, values of O(0.1)-O(1) are reasonable ! MLD
  !
  !  entmax    = upper limit of entrainment rate, prevents infinite
  !              entrainment rate if LCL is very low (recall entrainment
  !              rate is proportional to  entpar/z), units:  m^-1  ! MLD
  !
  !-----------------------------------------------------------------------
...
contains
  ...
  !######################################################################
  subroutine entraining_qe_moist_convection_init () ! 'entraining_' added by MLD
    ...
    if (file_exist('input.nml')) then
       ...
       ierr = 1
       do while (ierr /= 0)
          read  (unit, nml=entraining_qe_moist_convection_nml, iostat=io, end=10) ! 'entraining_' added by MLD
...
10     call close_file(unit)
    ...
    if ( mpp_pe() == 0 ) then
       ...
       write (unit,nml=entraining_qe_moist_convection_nml) ! 'entraining_' added by MLD
    endif
    call close_file(unit)

    do_init = .false.
    
    ! Calculates the size of the LCL lcl_temp_table with values Tmin
    ! Tmax
  ...
  end subroutine entraining_qe_moist_convection_init ! 'entraining_' added by MLD
  ...
  subroutine entraining_qe_moist_convection (dt, Tin, qin, p_full, z_full, p_half, coldT, &
                                  rain, snow, deltaT, deltaq, qref, convflag, &
                                  kLZBs, CAPE, CIN, invtau_q_relaxation, &
                                  invtau_t_relaxation, Tref) ! entraining_ and z_full added by MLD
    ...
    !           z_full   height at full model levels added by MLD (needed for
    !           entrainment paramter)
    !           p_half   Pressure at half (interface) model levels
    !           coldT    Flag indicating whether precipitation should be
    !           snow
    !                      (not used)
    !
    ...
    !                      full model levels
    !           convflag Flag indicating what kind of convection
    !           occurred:
    !                convflag = 0: no cape, no convection
    !                convflag = 1: shallow conv; predicted precip less
    !                than zero
    !                convflag = 2: deep convection
    !           kLZBs    Levels of zero buoyancy
    !           CAPE     Convectively available potential energy 
    !           CIN      Convective inhibition (this and the above are
    !           before the 
    !                      adjustment)
    !           invtau_q_relaxation
    ...
    real   , intent(in) , dimension(:,:,:) :: Tin, qin, p_full, z_full, p_half ! z_full added by MLD
    real   , intent(in)                    :: dt
    ...
    call entraining_SBM_convection_scheme(dt, Tin, qin, p_full, z_full, p_half, rain, snow, &
         deltaT, deltaq, kLZBs, CAPE, CIN,invtau_q_relaxation, &
         invtau_t_relaxation, Tref, qref,&
         val_min, val_max, val_inc, lcl_temp_table, convflag) ! entraining_ and z_full added by MLD
     
  end subroutine entraining_qe_moist_convection ! 'entraining_' added by MLD

  !#######################################################################
  
  subroutine entraining_SBM_convection_scheme(dt, Tin, qin, p_full, z_full, p_half, rain, snow, &
...
! entraining_ and z_full added by MLD
    ...
    real, intent(in), dimension(:,:,:)     :: Tin, qin, p_full, z_full ! z_full added by MLD
    ...
    do i=1, size(Tin, 1)
       do j=1, size(Tin, 2)
          ...
          call entraining_CAPE_calculation(k_surface, p_full(i,j,:), z_full(i,j,:), p_half(i,j,:), &
               Tin(i,j,:), rin(i,j,:), kLZB, T_parcel, r_parcel, &
               cape_parcel, cin_parcel, val_min, val_max, lcl_temp_table) ! entraining_ and z_full(i,j,:) added by MLD
          ...
          ! If CAPE>0, set reference temperature and humidity above and
          ! below 
          ...
          if (cape_parcel .gt. 0) then
             ...
             if ( (Pq_parcel .gt. 0) .and. (Pt_parcel .gt. 0) ) then
                ...
                if (Pt_parcel .gt. 0) then
                   ...
                   ! Else, do nothing, and go back to loop over latitude
                   ! and longitude 
                   Pq_parcel     = 0.
             ...
             ! If CAPE < 0, do nothing, and go back to loop over
  ...
  end subroutine entraining_SBM_convection_scheme ! entraining_ added by MLD; sbm -> SBM

  !#######################################################################
  
  subroutine entraining_CAPE_calculation(k_surface, p_full, z_full, p_half, Tin, rin, kLZB, &
       Tp, rp, CAPE, CIN, val_min, val_max, lcl_temp_table) ! entraining_ and z_full  added by MLD

    ! Calculates CAPE, CIN, level of zero buoyancy, and parcel
    ! properties 
    ! (second order accurate in delta(ln p) and exact LCL calculation)

    integer, intent(in)                  :: k_surface
    real, intent(in), dimension(:)       :: p_full, z_full ! z_full added by MLD 
    real, intent(in), dimension(:)       :: p_half
    ...
    ! Calculation of values to check whether the lowest level is
    ! saturated
    ...
    ! MLD note: no entrainment below LCL; entraining_CAPE_below_LCL represents
    ! entrainment at LCL but not below it
    call entraining_CAPE_below_LCL(saturated, k_surface, p_half, p_full, z_full, Tin, Tin_virtual, &
         Tp, T0, r0, rs, rp, rin, CAPE, nocape, CIN, pLZB, kLFC, kLZB, &
         pLCL, kLCL, skip, val_min, val_max, lcl_temp_table) ! entraining_ and z_full added by MLD
    
    ! Calculation above the LCL
    call entraining_CAPE_above_LCL(kLCL, kLZB, kLFC, Tp, rp, rin, p_full, z_full, nocape, skip,&
         CIN, CAPE, Tin, Tin_virtual, p_half, pLZB) ! entraining_ and z_full added by MLD
    
  end subroutine entraining_CAPE_calculation ! entraining_ added, CALCULATION -> calculation, MLD
  
  !#######################################################################

  subroutine entraining_CAPE_below_LCL(saturated, k_surface,  p_half, p_full, z_full,  Tin, Tin_virtual, &
                       Tp, T0, r0, rs, rp, rin, CAPE, nocape, CIN, pLZB, kLFC, kLZB, &
                       pLCL, kLCL, skip, val_min, val_max, lcl_temp_table) ! entraining_ and z_full added by MLD
! MLD note: subroutine name is misleading, there is not entrainment below the
! LCL, but there is at the level between the LCL and the nearest model level,
! as calculated in this subroutine
    ...
    real                               :: theta0, value, a, b , dtdlnp, es, entparz, entterm, rin_mid, tin_mid, z_mid ! entparz, entterm, and _mid terms added by MLD
    integer                            :: k
    real                               :: TLCL, zLCL, rin_LCL, Tin_LCL ! zLCL, rin_LCL, Tin_LCL added by MLD   
    ...
    if (saturated) then
       ...
       ! Saturate parcel (wring out excess moisture and change
       ! temperature 
       ...
       ! If the lowest level is not saturated, calculate temperature of
       ! saturation
       ...
       if (r0 .le. 0) then
          ...
          ! If the mixing ratio r0 > 0, calculate LCL temperature and
          ! temperature
          ...
          if (pLCL .lt. p_full(1)) then
             ! If the pLCL is above model domain, use values at top of
             ! the model
             pLCL = p_full(1)
             TLCL = theta0 * (pLCL/pref)**kappa
          end if
          
          ! Calculate parcel temperature and CIN below LCL by upward
          ! integration
          k   = k_surface
          CIN = 0.
          ...
          ! begin MLD addition
                ! calculate zlcl, rin_lcl, Tin_lcl by interpolation
                ! Temperature of parcel at LCL is TLCL,
                ! humidity of parcel at LCL is r0
          if (kLCL+1.gt.k_surface) then ! 'saturated' condition should prevent this if statement from being true. Still, this protects indexing in the 'else' portion of this statement
                zlcl = z_full(k_surface)
                rin_lcl = rin(k_surface)
                Tin_lcl = Tin(k_surface)
          else if (pLCL.eq.p_full(1)) then ! if LCL has been set to highest model level
                zlcl = z_full(1)
                rin_lcl = rin(1)
                Tin_lcl = Tin(1)
          else ! interpolate using neighboring model levels, linear in pressure
                rin_lcl = rin(kLCL) + (pLCL-p_full(kLCL))*(rin(kLCL+1)-rin(kLCL))/(p_full(kLCL+1)-p_full(kLCL))
                Tin_lcl = Tin(kLCL) + (pLCL-p_full(kLCL))*(Tin(kLCL+1)-Tin(kLCL))/(p_full(kLCL+1)-p_full(kLCL))
                zlcl = z_full(kLCL) + (pLCL-p_full(kLCL))*(z_full(kLCL+1)-z_full(kLCL))/(p_full(kLCL+1)-p_full(kLCL))
                ! check that Xlcl is between X(kLCL) and X(kLCL+1), for debugging
                if (((zlcl.lt.z_full(kLCL)).AND.(zlcl.lt.z_full(kLCL+1))).OR.((zlcl.gt.z_full(kLCL)).AND.(zlcl.gt.z_full(kLCL+1)))) then
                     write(*,*) 'zlcl not in between z(klcl) and z(klcl+1)'
                     call error_mesg ('qe_moist_convection','zlcl error', FATAL)
                end if
                if (((Tin_lcl.lt.Tin(kLCL)).AND.(Tin_lcl.lt.Tin(kLCL+1))).OR.((Tin_lcl.gt.Tin(kLCL)).AND.(Tin_lcl.gt.Tin(kLCL+1)))) then
                     write(*,*) 'Tin_lcl not in between Tin(klcl) and Tin(klcl+1)'
                     call error_mesg ('qe_moist_convection','Tin_lcl error', FATAL)
                end if
                if (((rin_lcl.lt.rin(kLCL)).AND.(rin_lcl.lt.rin(kLCL+1))).OR.((rin_lcl.gt.rin(kLCL)).AND.(rin_lcl.gt.rin(kLCL+1)))) then
                     write(*,*) 'rin_lcl not in between rin(klcl) and rin(klcl+1)'
                     call error_mesg ('qe_moist_convection','rin_lcl error', FATAL)
                end if
          end if
          ! end MLD addition

          ! Temperature profile for saturated ascent
          entparz = min(entmax,entpar/zLCL) ! added by MLD
          entterm = entparz/grav*(hlv*(r0-rin_lcl)+cp_air*(TLCL - Tin_lcl)) ! added by MLD
          a = (1+entterm)*kappa * TLCL + (HLv/Cp_air)*r0 ! (1+entterm)* added by MLD
          ...
          if ( (Tp(kLCL) .lt. Tmin) .and. nocape ) then
             ...
             rp(kLCL) = mixing_ratio(es, (p_full(kLCL) + pLCL)/2) ! rp and Tp on half level here (comment by MLD)
             rin_mid = (rin_lcl + rin(kLCL))/2
             tin_mid = (tin_lcl + tin(kLCL))/2
             z_mid = (zLCL + z_full(kLCL))/2
             entparz = min(entmax,entpar/z_mid) ! added by MLD
             entterm = entparz/grav*(hlv*(rp(kLCL)-rin_mid)+cp_air*(Tp(kLCL)-tin_mid)) ! added by MLD
             a = (1+entterm)*kappa * Tp(kLCL) + (HLv/Cp_air) * rp(kLCL) ! (1+entterm)* added by MLD
             ...
             if ( (Tp(kLCL) .lt. Tmin) .and. nocape) then
                ...
                call escomp(Tp(kLCL), es)
                rp(kLCL) = mixing_ratio(es, p_full(kLCL)) ! rp and Tp on full level here (comment by MLD)
  ...
  end subroutine entraining_CAPE_below_LCL ! entraining_ added by MLD

  !#######################################################################

  subroutine entraining_CAPE_above_LCL (kLCL, kLZB, kLFC, Tp, rp, rin, p_full, z_full, nocape_, skip, &
                      CIN, CAPE, Tin, Tin_virtual, p_half, pLZB) ! entraining_ and z_full added by MLD
    
    
    integer, intent(in)                    :: kLCL
    real, intent(in), dimension(:)         :: rin, p_full, z_full, Tin, Tin_virtual ! z_full added by MLD
    real, intent(in), dimension(:)         :: p_half
    ...
    integer                                :: k
    real                                   :: a, b, dtdlnp, es, entparz, entterm, rin_mid, tin_mid, z_mid  ! entparz, entterm, and _mid terms added by MLD
    logical                                :: nocape
    ...
    if (skip) then
       ...
       do k = kLCL-1, 1, -1
          entparz = min(entmax,entpar/z_full(k+1)) ! added by MLD, min prevents infinite entrainment parameter
          entterm = entparz/Grav*(hlv*(rp(k+1)-rin(k+1))+cp_air*(Tp(k+1)-Tin(k+1))) ! added by MLD
          a      = (1+entterm)*kappa*Tp(k+1) + (HLv/Cp_air) * rp(k+1) ! +entterm added by MLD
          ...
          if ( (Tp(k) .lt. Tmin) .and. nocape) then
             ...
             call escomp(Tp(k), es)
             rp(k) = mixing_ratio(es, ( p_full(k) + p_full(k+1) )/2) ! now rp and Tp on half level (comment by MLD)
             rin_mid = (rin(k) + rin(k+1))/2
             tin_mid = (tin(k) + tin(k+1))/2
             z_mid = (z_full(k) + z_full(k+1))/2
             entparz = min(entmax,entpar/z_mid) ! added by MLD, min prevents infinite entrainment parameter
             entterm = entparz/Grav*(hlv*(rp(k)-rin_mid)+cp_air*(Tp(k)-tin_mid)) ! added by MLD
             a = (1+entterm)*kappa * Tp(k) + (HLv/Cp_air)* rp(k) ! +entterm added by MLD
             ...
             if ( (Tp(k) .lt. Tmin) .and. nocape ) then
                ...
                if ( (virtual_temp(Tp(k), rp(k)) .lt. Tin_virtual(k) ) .and. nocape) then
                   ! If the parcel is not buoyant and does not yet have
                   ! CAPE, add to CIN
...
20  end if
  ...
  end subroutine entraining_CAPE_above_LCL ! 'entraining_' added by MLD
  ...
  real function virtual_temp(temp, r)
    ...
    ! Calculates the virtual temperature from the temperature and mixing
    ! ratio 
  ...
  subroutine level_of_zero_precip (kLZB, k_surface, deltaq, p_half, Pq, &
    ...
    ! Calculation of the precipitation up to one level below, until P >
    ! 0 
    ! or surface reached 
  ...
  subroutine do_deep_convection (kLZB, k_surface,Pt, dt,p_half,invtau_q_relaxation, &
    ...
    if (Pq.gt.Pt)then
       ...
       ! Do deep convection by changing the reference temperature
       ! profile
  ...
  subroutine do_change_Tref_deepconv(kLZB, k_surface, deltaT, deltaq, p_half, Tref,dt)
    ...
    ! Modification of the reference temperature profile in convective
    ! layer 
  ...
  subroutine get_lcl_temp(lcl_temp_table, value, val_min, val_max, Tlcl)
    ...
    ! Returns the approximation of the LCL temperature matching the
    ! value
    ...
    if (value .lt. val_min) then
       ...
       call error_mesg ('qe_moist_convection', 'get_lcl_temp: value too low.', FATAL)
    end if
  ...
  real function lcl_temp(value, lcl_temp_guess)
    ...
    ! unsaturated adiabatic ascent, at the LCL we have r(parcel) =
    ! r_sat(LCL). So  
    !
    ! T(LCL) = theta(LCL) (p(LCL)/pref)**kappa = theta(parcel)
    ! (p(LCL)/pref)**kappa, 
    !
    ! and, at the LCL, 
    !
    ...
    !      = rdgas/rvgas *es / [(T(LCL)/theta(parcel))**(1/kappa)*pref -
    !      es(LCL)]
    !
    ! Therefore, 
    !
    !            theta**(-1/kappa) * pref * r/(rdgas/rvgas + r) =
    !            es/T**(1/kappa).  (2)
    !
    ! On the LHS are parcel properties; on the RHS are properties of the
    ! LCL, which are 
    ! a function of temperature only. Given the LHS, we can solve for
    ! the temperature at 
    ! the LCL, which is what this function does.
    !
    ! The input value of this function (value) is the logarithm of the
    ! LHS of (2), 
    !
  ...
  subroutine entraining_qe_moist_convection_end ! entraining_ added by MLD
    
    deallocate (lcl_temp_table)
    
  end subroutine entraining_qe_moist_convection_end ! entraining_ added by MLD
  
  !##############################################################################
  
end module entraining_qe_moist_convection_mod
