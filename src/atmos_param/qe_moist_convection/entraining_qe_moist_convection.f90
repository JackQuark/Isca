...
10     call close_file(unit)
  ...
  subroutine entraining_qe_moist_convection (dt, Tin, qin, p_full, z_full, p_half, coldT, & ! entraining_ and z_full added by MLD
    ...
    !           z_full   height at full model levels added by MLD (needed for entrainment paramter)
    ...
    call entraining_SBM_convection_scheme(dt, Tin, qin, p_full, z_full, p_half, rain, snow, & ! entraining_ and z_full added by MLD
  ...
  subroutine entraining_SBM_convection_scheme(dt, Tin, qin, p_full, z_full, p_half, rain, snow, & ! entraining_ and z_full added by MLD
    ...
    do i=1, size(Tin, 1)
       do j=1, size(Tin, 2)
          ...
          call entraining_CAPE_calculation(k_surface, p_full(i,j,:), z_full(i,j,:), p_half(i,j,:), & ! entraining_ and z_full(i,j,:) added by MLD
  ...
  subroutine entraining_CAPE_calculation(k_surface, p_full, z_full, p_half, Tin, rin, kLZB, & ! entraining_ and z_full  added by MLD
    ...
    call entraining_CAPE_below_LCL(saturated, k_surface, p_half, p_full, z_full, Tin, Tin_virtual, & ! entraining_ and z_full added by MLD
         Tp, T0, r0, rs, rp, rin, CAPE, nocape, CIN, pLZB, kLFC, kLZB,     &
         pLCL, kLCL, skip, val_min, val_max, lcl_temp_table)
    
    ! Calculation above the LCL
    call entraining_CAPE_above_LCL(kLCL, kLZB, kLFC, Tp, rp, rin, p_full, z_full, nocape, skip,& ! entraining_ and z_full added by MLD
  ...
  subroutine entraining_CAPE_below_LCL(saturated, k_surface,  p_half, p_full, z_full,  Tin, Tin_virtual, & ! entraining_ and z_full added by MLD
                       Tp, T0, r0, rs, rp, rin, CAPE, nocape, CIN, pLZB, kLFC, kLZB, &
                       pLCL, kLCL, skip, val_min, val_max, lcl_temp_table)
